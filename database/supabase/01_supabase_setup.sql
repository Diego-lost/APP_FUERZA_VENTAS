...
-- ============================================================
-- INSTRUCCIONES:
-- Ejecutar en orden según database/supabase/README.md
-- 1. Ir a tu proyecto en supabase.com → SQL Editor
-- 2. Ejecutar primero 00_extensions.sql, luego este archivo
-- ============================================================
...
-- ── 6. Tabla: tarjetas (requerida por la app) ─────────────
CREATE TABLE IF NOT EXISTS public.tarjetas (
  id                 UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id            UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  tipo               TEXT NOT NULL CHECK (tipo IN ('debito','credito')),
  numero_enmascarado TEXT NOT NULL,
  estado             TEXT NOT NULL DEFAULT 'activa' CHECK (estado IN ('activa','apagada','bloqueada')),
  saldo_disponible   NUMERIC(12,2) NOT NULL DEFAULT 0,
  cuenta_asociada    TEXT NOT NULL,
  created_at         TIMESTAMPTZ DEFAULT now()
);

-- ── 7. Tabla: prestamos (requerida por la app) ────────────
...
DROP POLICY IF EXISTS "Usuario ve su propia cuenta de ahorro" ON public.cuentas_ahorro;
